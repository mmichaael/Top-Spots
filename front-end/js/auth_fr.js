...
// ── Зчитуємо ?mode= з URL і показуємо потрібну форму ──
function initModeFromUrl() {
    const params = new URLSearchParams(window.location.search);
    const mode = params.get('mode'); // 'login' або 'register'

    const regBlock  = document.getElementById('registratioBlock');
    const loginBlock = document.getElementById('loginBlock');
    if (!regBlock || !loginBlock) return;

    if (mode === 'login') {
        // показуємо логін, ховаємо реєстрацію
        regBlock.classList.remove('auth_active');
        regBlock.classList.add('auth_inactive', 'invalidRegLogForm');

        loginBlock.classList.remove('auth_inactive', 'invalidRegLogForm');
        loginBlock.classList.add('auth_active', 'validRegLogForm');
    } else {
        // default — реєстрація
        regBlock.classList.remove('auth_inactive', 'invalidRegLogForm');
        regBlock.classList.add('auth_active', 'validRegLogForm');

        loginBlock.classList.remove('auth_active');
        loginBlock.classList.add('auth_inactive', 'invalidRegLogForm');
    }
}
...
// Викликай одразу при завантаженні
initModeFromUrl();
